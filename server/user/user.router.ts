...
export class UserRouter extends BaseRouter {
  ...
  async uploadSubmission(req, res) {
    ...
    const upload_id = 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function (c) {
    ...
    const temporary = filename.split('.');
    const fileExtension = temporary[temporary.length - 1];
    ...
    const string = JSON.stringify(
      await this.service.writeToUploadsTable(
        upload_id,
        filename,
        file.size,
        fileExtension,
    ...
    const updated_upload_id = result[0].upload_id;
    return res.json(
      await this.service.writeToSubmissionTable(updated_upload_id, assignment.assignment_id, email),
    );
  }
  ...
  async submitGrading(req, res) {
    ...
    const teacher_email = req.session.passport.user;
    console.log(req.body.upload_id);
    const assignment_id = await JSON.parse(
      JSON.stringify(await this.service.getAssignmentID(req.body.upload_id)),
    ).assignment_id;
    console.log(assignment_id);
    const subject_id = await JSON.parse(
      JSON.stringify(await this.service.getSubjectID(assignment_id)),
    ).subject_id;
    console.log(subject_id);
    const marks = req.body.grade;
    const comment = req.body.comment;
    return res.json(
      await this.service.writeToMarksTable(
        student_email,
        teacher_email,
        assignment_id,
        subject_id,
