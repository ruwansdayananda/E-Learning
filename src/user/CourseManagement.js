  ...
  return (
    <div style={{margin: '100px'}}>
      ...
      >
        <Grid container>
          <Grid item xs={7}>
            <FormInput name="addSubject" label="Type new subject" type="text" className={classes.formContent} fullWidth onChange={(event) => {
          ...
          <Grid container>
            ...
            <Grid item xs={6} style={{padding: '0 50px'}}>
              ...
              <Divider style={{margin: '30px 0'}}/>
              {currentSubjects.map((sub)=> <Card style={{margin: '10px', padding: '10px'}} variant="outlined">
            ...
            <Grid item xs={6} style={{padding: '0 50px'}}>
              ...
              <Divider style={{margin: '30px 0'}}/>
              {addSubject.map((sub)=> <Card style={{margin: '10px', padding: '10px'}} variant="outlined">
            ...
            <Grid container>
                ...
                <Grid item xs={1}>
                    <Button
                        ...
                        className={classes.add}
                        onClick={() => {
                            setAddSubjectGrade([...addSubjectGrade , {grade_id: selectedGrade.grade_id , subject_id: selectedSubject.subject_id}]);
                        }}
                    >
                        Add
                    </Button>
                ...
                {showGradeSubjectAlert && <Alert severity="success">Subjects added to the grades successfully</Alert>}
                <Grid container>
                    <Grid item xs={12} >
                        <Typography variant='h5'>Subjects to be added to the selected grade</Typography>
                        <Divider style={{margin: '30px 0'}}/>
                        {currentGrades.map((val) => val.grade_id == selectedGrade.grade_id && <Typography variant='h6'>{val.grade}</Typography>)}
                        <Divider style={{margin: '30px 0'}}/>
                        {addSubjectGrade.map((item)=>
                            <Card style={{margin: '10px', padding: '10px'}} variant="outlined">
                            {currentSubjects.map((val) => val.subject_id == item.subject_id && <Typography variant='h6'>{val.subject}</Typography>)}
                            </Card>
