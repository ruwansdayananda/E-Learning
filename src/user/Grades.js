...
export const Grades = () => {
  ...
  const subject = marksInformation.subject_id;
  const assignmentTitle = marksInformation.assignment_id;
  const marks = marksInformation.marks;
  const gradedBy = marksInformation.teacher_email;
  function FormRow(){
    return(
      <React.Fragment>
        <Grid item xs={6}>
  ...
  function createData(subject, a_title, grade, graded_by) {
    return { subject, a_title, grade, graded_by };
  }

  const rows = [
        createData(subject, assignmentTitle, marks+'/100', gradedBy),
  ];
  return (
          ...
          <TableBody>
            {rows.map((row) => (
              <StyledTableRow key={row.subject}>
                ...
                <StyledTableCell align="center">{row.a_title}</StyledTableCell>
                <StyledTableCell align="center">{row.grade}</StyledTableCell>
                <StyledTableCell align="center">{row.graded_by}</StyledTableCell>
              </StyledTableRow>
            ))}
          </TableBody>
        </Table>
